getModificationPosition_MQ <- function(mod_seq){ 
  ...
  ## EXAMPLE SEQUENCES
  # strings <- c("_EALPM(ox)DTEVYESPpYADPEEIRPK_", 
  #              "_VKEEGYELPYNPATDDpYAVPPPR_", 
  #              "_ILpSDVTHpSAVFGVPASK_", 
  #              "_HSM(ox)WpYHGPVpSR_")
  
  # Pattern and replacement
  pattern_ox <- "\\(ox\\)"
  replacement_ox <- "(Oxidation (M))"
  modified_string <- gsub(pattern_ox, replacement_ox, mod_seq)
  
  pattern_p <- "p"
  replacement_p <- "(Phospho (STY))"
  modified_string1 <- gsub(pattern_p, replacement_p, modified_string)
  
  pattern_ac <- "\\(ac\\)"
  replacement_ac <- "(Acetyl (Protein N-term))"
  final_string <- gsub(pattern_ac, replacement_ac, modified_string1)
  
  mod_seq1 <- substr(final_string, 2, nchar(final_string)-1)
  ...
  for ( mod in unique(modification_labels) ) {
    ...
    # pos_mod <- base::gregexpr( "@", current_mod_sequence )
    # current_mod_sequence <- "HSMW@YHGPV@SR"
    pos_mod <- gregexpr("@", current_mod_sequence)[[1]]
    #positions <- ifelse(matches == -1, NA, matches)
    
    if (mod == "(Phospho (STY))"){
      if ( length(as.numeric(unlist((pos_mod))))== 1 ){
      ...
      #   pos_mod <-as.numeric(unlist((pos_mod))) 
        ...
        pos_mod[1] <-as.numeric(unlist((pos_mod)))[1]
        pos_mod[2] <- as.numeric(unlist((pos_mod)))[2]-1
        ...
        pos_mod[1] <-as.numeric(unlist((pos_mod)))[1]
        pos_mod[2] <- as.numeric(unlist((pos_mod)))[2]-1
        pos_mod[3] <- as.numeric(unlist((pos_mod)))[2]-2
      }
    }else if (mod == "(Oxidation (M))"){
      if ( length(as.numeric(unlist((pos_mod)))) == 1){
        pos_mod[1] <-as.numeric(unlist((pos_mod)))[1] -1
      #   
      # }else if (length(as.numeric(unlist((pos_mod))))==1){
      #   pos_mod <-as.numeric(unlist((pos_mod))) -1
      #   
    ...
    else if (mod == "(Acetyl (Protein N-term))"){
