...
getModificationPosition_general <- function( mod_seq, software_name){ 
  ...
  if (software_name == "MQ_1634"){
    source("D:/dev/Desktop_copy/PHD/data_analysis/scripts/getModificationPositionMQ_func_edit_v1_1634.R")
    getModificationPosition_MQ(mod_seq)
    return( modification_index_list )}
  
  else if (software_name == "DIANN"){
    source("D:/dev/Desktop_copy/PHD/data_analysis/scripts/final_used_functions/getModificationPosition_func_for_all_mods.R")
    getModificationPosition_( mod_seq, character_index=F)
  }
