...
final_maxquant_214_pep_quant_analysis_bio <- function(file_path,
                                                 file_name,
                                                 #sheet_name,
                                                 #theo_file_path,
                                                 #theo_file_name,
  ...
  df_wide <- df_reversible %>% 
    ...
    relocate(contains(exp_design), .after = where(is.character))
  ...
  p6 <- gg_boxplt_exp_ratio(data_set = df_FC_ratio_after_impt, 
                            ...
                            fill_lab = "Sample Names",
                            subtitle_txt =  paste0("Experiment - ", exp_id, acquisiton_type, " data processed by ", software_name))
  ...
  if(test_type== "t.test" | test_type== "wilcoxon"){
    ...
    p9 <- ggplot(merge_stat_df ,aes(x =log2(fold_change_values), y = -log10(pvalues), color=species)) +
  ...
  p9 <- gg_volcano(data_set = merge_stat_df,
                    x_df = merge_stat_df$logFC,
                    y_df = -log10(merge_stat_df$P.Value),
                    color_df = merge_stat_df$accession,
                    facet_df = "...8",
