...
final_data_log10_abun <- final_selected_abun_df_merge %>%
  mutate(log10_int = log10(Intensity)) %>% select(-Intensity) %>%
  pivot_wider(names_from = Experiment, values_from = log10_int, names_glue = "{Experiment}")

df_filtered <- final_data_log10_abun %>%
  filter(rowSums(is.na(.)) <= 10) %>% 
  pivot_longer(names_to = Experiment, values_to = Intensity) %>%
  mutate(Intensity = case_when(
  Sample_id == "A1" ~ if_else(is.na(Intensity), log10(as.numeric(quantile_values[1,2]), Intensity)),
  Sample_id == "A2" ~ if_else(is.na(Intensity), log10(as.numeric(quantile_values[2,2]), Intensity)),
  Sample_id == "A3" ~ if_else(is.na(Intensity), log10(as.numeric(quantile_values[3,2]), Intensity)),
  Sample_id == "A4" ~ if_else(is.na(Intensity), log10(as.numeric(quantile_values[4,2]), Intensity)),
  Sample_id == "A5" ~ if_else(is.na(Intensity), log10(as.numeric(quantile_values[5,2]), Intensity))
)) %>% pivot_wider(names_from = Experiment, values_from = log10_int, names_glue = "{Experiment}")
...
test <- id_syn_phospho_pep %>% select(Intensity) %>%
  pivot_wider(names_from = Experiment, values_from = log10_int, names_glue = "{Experiment}")
...
for(j in 1:dim(data_wide)[1]){
  
  p_values_12[j] <- ttest_func(data_wide[j,2:4], data_wide[j,5:7])
  p_values_13[j] <- ttest_func(data_wide[j,2:4], data_wide[j,8:10])
  p_values_14[j] <- ttest_func(data_wide[j,2:4], data_wide[j,11:13])
  p_values_15[j] <- ttest_func(data_wide[j,2:4], data_wide[j,14:16])
}
