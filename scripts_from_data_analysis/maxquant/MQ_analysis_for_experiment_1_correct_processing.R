...
final_MQ_pep_quant_analysis_exp1 <- function(file_path,
  ...
  if (background_species %in% c("ECOLI","Escherichia coli")){
    ...
    write.table(comb_ecoli_dist,file=paste0(new_path,"/", #file_path,curr_dir
                                            "exp",
  ...
  final_results_with_common_col <- phospho_tmp %>%
    ...
    mutate(ptm_score = sapply(str_extract_all(Phospho..STY..Probabilities,
                                              "\\(\\d+(\\.\\d+)?\\)"), function(x) {
           ...
           # mutate(
           #   ptm_score = sapply(str_extract_all(`Phospho (STY) Probabilities`, "\\(\\d+\\.\\d+\\)"), function(x) {
           #     values <- as.numeric(str_extract_all(x, "\\d+")) #.\\d+
           #     if (length(values) == 0) NA else max(values)
           #   })
    )%>% relocate(ptm_score,.after = Sequence)
  #drop_na(ptm_score) %>%
  ...
  comb_result_pos <- final_results_with_common_col %>% 
    ...
    select(Sequence, pep_with_pos,ptm_score,Positions,
           Experiment,raw_file, #### !!!  Q.Value is only necessary for 100% FDR RUNS !!! ####
  ...
  comb_result_pep_filt <- comb_result_pep_poolwise %>% filter(ptm_score >= 0.75)
  ...
  if(background_species == "ECOLI"){
    ...
    comb_result_pep_filtered <- comb_result_pep_poolwise %>% 
      separate(Experiment,into = c("exp","samp","coli","inj"),sep = "-",remove = F) %>%
  ...
  sapply(1:length(plt_obj),function(x) ggsave(filename = paste0("p",x,".png"),
                                              width = 60, height = 45, 
                                              path = paste0(file_path,curr_dir,"/output_final_aft_mapp_func/"), #output_final
                                              units = "cm",
                                              get(plt_obj[x]),
                                              device = "png", #".svg"
  ))
