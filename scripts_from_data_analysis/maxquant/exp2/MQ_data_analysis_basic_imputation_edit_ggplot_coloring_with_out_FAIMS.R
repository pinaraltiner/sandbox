...
file_path <- "D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/MQ_data_analysis/exp2_wo_FAIMS/exp2_wo_FAIMSwith_MBR/"

#file_path <- "D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/MQ_data_analysis/with_FAIMS_MBR"
setwd(file_path)
...
## with FAIMS
p_thresholds <- c(0.0328029479,3.877033e-02,5.321280e-02,3.894957e-02)
...
volcano_final1 <- volcano_final %>% select(-Ratio_value) %>%
  ...
  #write.table(volcano_final1,file = paste0(file_path,"/","exp2wo_FAIMS_DDA_MQ_volcano_plot_data.tsv"),sep = "\t",row.names = F)
...
ggplot(volcano_final1, aes(x = log2(exp_FC_value), y = -log10(pvalues_value))) +
  ...
  labs(title = "Experiment 2 - DDA no FAIMS processed by MaxQuant", color = "Classes", shape="Type")
