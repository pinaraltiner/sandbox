...
final_proline_pep_quant_analysis <- function(file_path,
  ...
  ggplot(figure_with_mean_abundance, 
         aes(x= factor(Theo_Exp),y = log10(Mean_abundance),fill=factor(Pool))) +
    ggdist::stat_halfeye(
      adjust = 0.5,
      #justification = -2,
      .width = 0,
  ...
  colnames(p_values_for_all_ratio)[-1] <- sapply(2:5, function(x) paste0("p_values_for_ratio",x)) 
  
  #p_values_for_all_ratio$common_col_for_merging <- df_for_ttest$common_col_for_merging
