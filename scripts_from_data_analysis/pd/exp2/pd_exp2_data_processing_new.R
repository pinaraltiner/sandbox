...
file_path <- "D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/PD_data_analysis/target_decoy_no_FAIMS/new/"
file_name <- "exp2_wo_faims_target_decoy_pd_correct_phospho_pos.xlsx"
...
theo_file_name <- "Synthetic peptides list_theo_conc_added_pool_id_iso_count_add_pep.xlsx" 
...
final_pd_pep_quant_analysis <- function(file_path,
                                             ...
                                             theo_file_name,
                                             sheet_theo_name){
  exp2_noFAIMS_DDA_pd <-read.xlsx(paste0(file_path,file_name),sheet = sheet_name)
  ...
  ### IMPUTATION
  abundances_for_impute <- exp2_noFAIMS_DDA_pd %>% 
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  write.table(roc_final,file = paste0(file_path,"/","ROC_curve_data_PD_target_decoy_noFAIMS.txt"),sep = "\t",row.names = FALSE)
