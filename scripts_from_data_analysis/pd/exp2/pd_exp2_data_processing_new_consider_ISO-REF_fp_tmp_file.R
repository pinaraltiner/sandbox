...
final_pd_pep_quant_analysis <- function(file_path,
  ...
  ttest_func <- function(x, y) {
      ...
      #   
      # }
      t.test(x, y,alternative = c("two.sided"))$p.value #var.equal=TRUE
  }
  ...
  if(test_type== "t.test" | test_type== "wilcoxon"){
      ...
      p9<- merge_stat_df %>% filter(!grepl("ECOLI", Pool)) 
          ggplot(p9,aes(x =log2(fold_change_values), y = -log10(pvalues))) +
          ...
          scale_y_continuous(limits = c(0, max(-log10(p9$pvalues))), breaks = seq(0, max(-log10(p9$pvalues)), by = 0.8)) +
          scale_x_continuous(limits = c(min(log2(p9$fold_change_values)),max(log2(p9$fold_change_values)))) +#facet_wrap(~ratio) +
      ...
      p9_limma <- merge_stat_df_final %>% filter(!grepl("ECOLI", Pool)) %>%
          ggplot(aes(x =log2(fold_change_values), y = -log10(adj_pvalues))) +
          geom_point(aes(color = new_col_coloring,shape=Pool_new), size = 2.5) +
          #geom_line(aes(color = new_col_coloring), size = 1) +  # Add color aesthetic to geom_line()
          scale_color_manual(values = c("ISO-REF" = "#000000",
                                        "Unexpected_False Positive_A1/A2"="#999999",
          ...
          # scale_shape_manual(values = c(16, 15, 12, 17),
          # labels = c('Non-variant', 'Variant isomeric', 'Variant non-isomeric', 'Unexpected')) +
          scale_y_continuous(limits = c(0, max(-log10(pvalues))), breaks = seq(0, max(-log10(adj_pvalues)), by = 0.8)) +
          scale_x_continuous(limits = c(min(log2(fold_change_values)),max(log2(fold_change_values)))) +#facet_wrap(~ratio) +
          #scale_y_continuous(limits = c(0, 8), breaks = seq(0, 8, by = 0.8)) +
          #scale_x_continuous(limits = c(-3,3),breaks = seq(-3, 3, by = 0.8)) +
          #scale_y_continuous(breaks = seq(0, max(-log10(volcano_final1$pvalues_value)), length.out = 21)) +
          theme_bw() +
  ...
  selected_cols <- colnames(exp2_noFAIMS_DDA_pd[c(5,7,16,26)])
  #selected_cols <- colnames(exp2_noFAIMS_DDA_proline[c(1:4,15)])
  
  
  syn_phospho_pep_pd <- exp2_noFAIMS_DDA_pd %>% 
    select(-starts_with("Abundances.Normalized")) %>% 
    mutate(abundances_for_impute) %>%
    filter(grepl("Phospho", Modifications)) %>%
    filter(grepl("PhosphoFerries", Marked.as)) %>% 
    filter(!grepl("positions",Modification.Pattern)) %>%
   # select(contains(selected_cols) | starts_with("abundance"))
    select((contains(selected_cols) | starts_with("Abundances.Normalized")))
  ...
  # phospho_ptm_pos <- lapply(syn_phospho_pep_pd$ons, function(each_ptm_protein_positions) {
  #   
  ...
  common_col_exp_quant <- as.data.frame(paste(syn_phospho_pep_pd$Sequence, syn_phospho_pep_pd$correct_mod, sep = "_"))
  colnames(common_col_exp_quant) <- "common_col_for_merging"
  
  # Bind it to the quant data
  syn_phospho_pep_pd_new <- cbind(common_col_exp_quant,syn_phospho_pep_pd)
  ...
  sorted_colnames <- c("Abundances.Normalized") # Abundance contains both w/wo raw_abundance
  for (i in 1:length(sorted_colnames)){
    assign(paste0("tmp",i), sort(colnames(select(syn_phospho_pep_pd_new,
                                                 starts_with(sorted_colnames[i])))))
  }
  
  sort_col <- c(tmp1)
  
  sort_syn_phospho_pep_pd_new <- syn_phospho_pep_pd_new 
  colnames(sort_syn_phospho_pep_pd_new)[8:22] <- experiment_name  
  ...
  syn_phospho_pep_pd_unique_max_absum <- sort_syn_phospho_pep_pd_new %>% 
  ...
  df_merge <- syn_phospho_pep_pd_unique_max_absum %>% left_join(pep_list_w_theo_quant_new,by="common_col_for_merging")
  ...
  for (k in 1:sample_size){
    ...
    assign(paste0("log2_abundances_A",k),as.data.frame(log2(df_merge  %>% select(contains(paste0("E2_A",k)))))) 
    
    
    filtered_abundances_rowMeans<- bind_cols(filtered_abundances_rowMeans,get(paste0("row_means_abundances_A",k)))
    
    filtered_abundances_log2 <- bind_cols(filtered_abundances_log2, get(paste0("log2_abundances_A",k)))
  ...
  filtered_abundances_log2 <- filtered_abundances_log2 %>%
    rename_with(~ paste0("log2_", .x), everything())# %>%
  ...
  final_imputed_data <- cbind(df_merge,filtered_abundances_rowMeans,filtered_abundances_log2)
   #write.table(final_imputed_data,file="D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/PD_data_analysis/target_decoy_no_FAIMS/new/PD_after_merging_with_correct_syn_list.tsv",sep = "\t")
  ...
  stat_analysis <- final_imputed_data %>%
    select(common_col_for_merging | starts_with("log2_E2"))
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  p_thresholds<- c(3.138610e-02,3.607779e-02,3.662885e-02,3.701608e-02)
  ...
  roc_data121 <- cbind(roc_data12, "A1 vs A2","Others", "PD")
  roc_data131 <- cbind(roc_data13, "A1 vs A3","Others", "PD")
  roc_data141 <- cbind(roc_data14, "A1 vs A4","Others", "PD")
  roc_data151 <- cbind(roc_data15, "A1 vs A5","Others", "PD")
