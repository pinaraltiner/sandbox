...
final_pd_pep_quant_analysis <- function(file_path,
  ...
  if(test_type== "t.test" | test_type== "wilcoxon"){
      ...
      p9 <- merge_stat_df_final %>% ggplot(aes(x =log2(fold_change_values), y = -log10(merge_stat_df_final$P.Value))) +
          geom_point(aes(color = new_col_coloring,shape=Pool_new), size = 2.5) +
          scale_color_manual(values = c("ISO-REF" = "#000000",
                                        "Unexpected_False Positive_A1/A2"="#999999",
                                        "Unexpected_False Positive_A1/A3" ="#999999",
          ...
          geom_vline(data = actual_ratio_col, aes(xintercept = log2(actual_ratio_val), show.legend = FALSE),color=c("#CC79A7","#E69F00","#56B4E9","#009E73"),size=1) + #color=c("#CC79A7","#E69F00","#56B4E9","#009E73")
          geom_hline(yintercept = -log10(fdr_threshold), linetype = "dashed", color = "red",size=1)
  ...
  write.table(roc_plot_df, file = paste0(file_path,"outputs_with_new_script/","Roc_analysis_",exp_id,"_",software_name,"_",acquisiton_type,".txt"),sep = "\t",row.names = F)
