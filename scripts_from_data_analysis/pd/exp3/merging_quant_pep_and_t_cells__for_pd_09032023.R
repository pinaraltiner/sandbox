...
library(reshape2)
###############################################
## DATA NAMING IS DIFFERENT THAN PROLINE, I CHANGED IT MANUALLY, ALSO ".na"
## WRITTEN IN THE REPORT.

## FOR EXPERIMENT 3, DO NOT NEED ANY MERGING
                  ## ONLY USED FILTERING BASED ON "PHOSPHO" and "MOUSE"


setwd("D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_3/PD_data_analysis")
quant_peptides <- read.table("Multiconsensus_TCellsExp3_230224_PeptideGroups.txt", sep = "\t", header = T)

quant_peptides_with_all <- quant_peptides %>% 
  filter(grepl("Mus musculus",Master.Protein.Descriptions)) %>% 
  filter(grepl("Phospho",Modifications)) 

data_abundace <- quant_peptides_with_all %>% select(contains("Normalized"))
...
## TO REPRESENTATION OF NUM OF IDENTIFIED PEPTIDES ACROSS ALL SAMPLES
quant_peptides_s1 <- na.omit(data_abundace[,c(1:3)])
quant_peptides_s2 <- na.omit(data_abundace[,c(4:6)])
quant_peptides_s3 <- na.omit(data_abundace[,c(7:9)])
quant_peptides_s4 <- na.omit(data_abundace[,c(10:12)])
quant_peptides_s5 <- na.omit(data_abundace[,c(13:15)])
