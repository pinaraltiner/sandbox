...
final_pd_pep_quant_analysis_bio <- function(file_path,
                                                 file_name,
                                                 sheet_name,
                                                 theo_file_path,
                                                 theo_file_name,
    ...
    df_mean_ab_after_impt <- final_imputed_data %>% 
        select(contains("aft_imp") | contains("Marked.as")) %>%
        tibble() %>% 
    ...
    df_FC_ratio_after_impt <- final_imputed_data %>% 
        select(starts_with("exp_")| contains("Marked.as")) %>%
        #separate(accession, into = c("uniprot_id", "species"), remove = F) %>%
        tibble() %>% 
    ...
    p4 <- gg_density(data_set = df_mean_ab_after_impt, 
                     ...
                     color_df = df_mean_ab_after_impt$Marked.as,
    ...
    p5 <- gg_density(data_set = df_FC_ratio_after_impt,
                     ...
                     color_df = df_FC_ratio_after_impt$Marked.as,
    ...
    p6 <- gg_boxplt_exp_ratio(data_set = df_FC_ratio_after_impt, 
                              ...
                              fill_df = df_FC_ratio_after_impt$Marked.as,
