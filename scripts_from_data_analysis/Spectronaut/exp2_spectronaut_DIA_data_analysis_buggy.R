...
###############################################
source("/home/claude/post_processing/post_processing_parquet/additional_functions/ggplot_functions.R")
...
source("/home/claude/post_processing/post_processing_parquet/additional_functions/roc_curve_generation_proline_edit_fixed.R")
...
final_spectronaut_pep_quant_analysis_syn <- function(file_path,
  ...
  intended_dir <-paste0(file_path,"outputs_imputed_mice_one_sample_nonNA/")
  ...
  find_max_value_and_pos <- function(ptm_count, ptm_prob, ptm_pos) {
    ...
    if (dim_count >= 2) {
      ...
      for (i in 1:length(max_pos)){
        ...
        probs = paste0("prob",i)
        poses = paste0("pos",i)
        result[[probs]] <- append(result[[probs]], paste(max_prob[i]))
        result[[poses]] <- append(result[[poses]], paste(max_pos[i]))
  ...
  ## EXPERIMENTAL DATA
  ## It was crashing with speclib output of Spectronaut: 
  ## R session crashing when reading a bunch of csv files when one of the files has no rows · Issue #1297 · tidyverse/readr · GitHub
  #quant_peptides <- read_tsv(paste0(file_path,file_name))
  
  quant_peptides <- fread(paste0(file_path,file_name))
  ##IMPUTED DATA
  ...
  ## Gaussian-modeled 5th percentile for imputation (Issue 14 fix)
  ##
  ## Rationale: the empirical 5th percentile is sensitive to the shape of the lower
  ## intensity tail, which differs between tools (DIA-NN vs Spectronaut). Instead,
  ## we model log10(Intensity) as a normal distribution and derive P5 analytically
  ## from two robust estimators: the median (= mu) and the IQR (-> sigma).
  ##
  ## Mathematical derivation (all in log10 space):
  ##   1. For a normal distribution, median = mu
  ##   2. IQR = Q75 - Q25 = (mu + 0.6745*sigma) - (mu - 0.6745*sigma) = 1.3490*sigma
  ##      => sigma = IQR / 1.3490
  ##   3. The 5th percentile = mu - z_0.95 * sigma, where z_0.95 = qnorm(0.95) = 1.6449
  ##      => P5_log10 = mu - 1.6449 * sigma
  ##   4. Convert back to linear scale: P5 = 10^P5_log10
  ##
  ## Both median and IQR have a breakdown point of 0.25, making them insensitive
  ## to up to 25% contamination from low-intensity noise or high-intensity outliers.
  ## This replaces the MICE imputation that was previously used for Spectronaut
  ## (see disabled block at line ~863), ensuring both tools use the same method.
  imputed_values <- quant_peptides_with_cond %>%
    filter(!is.na(Intensity) & Intensity > 0) %>%
    mutate(log10_int = log10(Intensity)) %>%
    group_by(Experiment) %>%
    summarise(
      mu_log10       = median(log10_int),
      sigma_log10    = IQR(log10_int) / 1.3490,
      P5_log10       = median(log10_int) - 1.6449 * (IQR(log10_int) / 1.3490),
      first_quantile = 10^(median(log10_int) - 1.6449 * (IQR(log10_int) / 1.3490)),
      .groups = "drop"
    )
  ...
  pep_list_w_theo_unique <- pep_list_w_theo %>% 
    ...
    rename(PEP.GroupingKey = Phosphopeptide.sequence) %>%
  ...
  all_seq <- quant_peptides_with_cond %>% 
    ...
    filter(grepl(selected_spcies,PG.ProteinLabel) & !grepl("CON__",PG.ProteinLabel)) %>%
  ...
  all_corr_seq <- quant_peptides_with_cond %>% 
    ...
    select(PEP.GroupingKey,EG.PrecursorId,Experiment,situation,Pool_for_seq_merge) %>%
  ...
  if(str_detect(file_path,pattern = "ProlineLib") | str_detect(file_path,pattern = "PDLib")){
    #if (file_name == "Exp2_redesigned_DIAnoFAIMS_usingPercoPDLib_Exp1wEcoli_Report.tsv" | 
    #file_name == "Exp2_redesigned_DIAnoFAIMS_usingProlineLib_Exp1wEcoli_Report.tsv")
    #"Exp2_redesigned_DIAnoFAIMS_usingSpecLibFromProlineAftExportFromSpect_rmv_IntCols_Report.tsv"){#"){
    
    # Apply the function to each row
    max_prob_and_pos <- apply(quant_phospho, 1, function(row) {
      find_max_value_and_pos(ptm_prob = row["EG.PTMProbabilities [Phospho]"],
                             ptm_pos = row["EG.PTMPositions [Phospho]" ],
  ...
  #######################################################################
  barplt_phospho_seq <- quant_phospho_combined %>%
  ...
  barplt_prot_ecoli <- quant_peptides_with_cond %>% 
    ...
    group_by(PG.ProteinLabel,Experiment) %>% ## sample_rep_id_seq allowed us to keep one sequence for each sample
    slice(which.max(Intensity)) %>%
    ungroup() %>%
  ...
  barplt_df_wide_all_col <- barplt_df %>%
    ...
    left_join(site_prob_tmp, by="pep_with_pos") %>%
    mutate(species=selected_spcies) 
  ...
  ## ATTENTION: SINCE OUTPUT OF SPECTRONAUT WAS ALREADY FILTERED BEFORE EXPORTING THE OUTPUT FILES
  ## DOES NOT MATTER THE LOCATION OF THIS CHUCK OF CODE (either before or after filtering apply) 
  ## UNLESS LOC_FILTER WAS SET DIFFERENT THAN 0.75
  
  # Nothing is changed
  filtered_abundances<-barplt_df_wide[rowSums(!is.na(select(barplt_df_wide,starts_with(exp_design))))>0,] #3
  filtered_abundances_ecoli <-barplt_df_ecoli_wide[rowSums(!is.na(select(barplt_df_ecoli_wide,starts_with(exp_design))))>0,] #3
  ...
  # }
  # 
  # filtered_abundances <-  data_filter_func(df = barplt_df_wide,samp_names = sample_names,num_allowed_NA = 1,num_expected_nonNA = 3)
  # filtered_abundances_ecoli <-  data_filter_func(df = barplt_df_ecoli_wide,samp_names = sample_names,num_allowed_NA = 5,num_expected_nonNA = 1)
  # 
  ...
  df_merge_all_col <- filtered_abundances %>%
  ...
  #    library(kableExtra) # DISABLED: only used in commented-out code
  ...
  phospho_completeness <- filtered_abundances %>% 
    count(na_val) %>%
    mutate(data_complete=((n/dim(filtered_abundances)[1])*100)) %>% mutate(species=selected_spcies)
  ...
  barplt_phospho_seq <- filtered_abundances %>%
    ...
    mutate(sample_rep_id_seq = paste(PEP.GroupingKey, Sample_id,Rep_id, sep = "@")) %>%
  ...
  # 
  # library("mice")
  # mice.impute.norm(y=filtered_abundances_bef_impt,ry=test,x=length(filtered_abundances_bef_impt))
  # 
  # x <- model.matrix(~ x1 + x3, data = data) 
  ...
  #filtered_abundances_bef_impt <- filtered_abundances
  #filtered_abundances_ecoli_bef_impt <- filtered_abundances_ecoli
  
  #######################################################
  #### Impute missing values with percentatile #######"
  exp_cols <- which(colnames(filtered_abundances) %in% exp_design)
  exp_cols_ecoli <- which(colnames(filtered_abundances_ecoli) %in% exp_design)
  for (j in 1:length(imputed_values_vec)){
    filtered_abundances[,exp_cols[j]][is.na(filtered_abundances[,exp_cols[j]])] <- imputed_values_vec[j]
    filtered_abundances_ecoli[,exp_cols_ecoli[j]][is.na(filtered_abundances_ecoli[,exp_cols_ecoli[j]])] <- imputed_values_vec[j]
  ...
  abundances_all_aft_imputation <- filtered_abundances_ecoli %>%
    rename(pep_with_pos=PEP.GroupingKey) %>%
    bind_rows(filtered_abundances)
  
  #######################################################
  #### Impute missing values with Gaussian Distribution  #######"
  #filtered_abundances <- as.data.frame(filtered_abundances)
  #rownames(filtered_abundances) <-  paste(filtered_abundances$pep_with_pos,
  #filtered_abundances$PG.ProteinLabel,
  #filtered_abundances$max_value,
  #filtered_abundances$species,sep ='@')
  
  #filtered_abundances_bef_impt <- filtered_abundances %>% select(pep_with_pos, contains("E2-A"))
  # 
  
  # 
  # 
  # df <- impute_background_noise_gaussian(filtered_abundances_bef_impt)[[3]]
  # df_ecoli <- impute_background_noise_gaussian(filtered_abundances_ecoli_bef_impt)[[3]]
  # 
  # library(imputeLCMD)
  # all_filt_aft_impt <- NULL
  # for (i in 1:sample_size){
  #   
  #   sel_cols <- filtered_abundances_bef_impt %>% select(contains("E2-A",i))
  #   sel_cols_ecoli <- filtered_abundances_ecoli_bef_impt %>% select(contains("E2-A",i))
  #   filtered_abundances_aft_impt <-   impute.MinProb(sel_cols, q = 0.01, tune.sigma = 1)
  #   filtered_abundances_ecoli_aft_impt <- impute.MinProb(filtered_abundances_ecoli_bef_impt, q = 0.01, tune.sigma = 1)
  #   
  #   all_filt_aft_impt <- bind_rows(all_filt_aft_impt, filtered_abundances_aft_impt,filtered_abundances_ecoli_aft_impt)
  #   rm(sel_cols)
  # }
  
  
  # dataSet.mvs <- filtered_abundances_bef_impt
  # 
  # function (filtered_abundances_bef_impt, q = 0.01, tune.sigma = 1) 
  # {
  ...
  # 
  # ## PHOSPHO PART ##
  # filtered_abundances_df <- as.data.frame(filtered_abundances)
  # 
  # rownames(filtered_abundances_df) <- paste(filtered_abundances_df$pep_with_pos,filtered_abundances_df$PG.ProteinLabel,
  #                                           filtered_abundances_df$max_value,
  #                                           filtered_abundances_df$species,sep = "@")
  # 
  # filtered_abundances_df <- filtered_abundances_df %>% select(contains("E2-"))
  # 
  # filtered_abundances_imp <- missForest(xmis=as.data.frame(filtered_abundances_df),verbose = TRUE)
  # filtered_abundances_imp_df <- as.data.frame(filtered_abundances_imp[["ximp"]])
  # 
  # filtered_abundances_final <- filtered_abundances_imp_df %>% 
  #   bind_cols(rownames(filtered_abundances_df)) %>%
  #   separate(col = 19,into = c("pep_with_pos","PG.ProteinLabel","max_value","species"),sep = "@")
  # 
  # ## ECOLI PART ##
  # filtered_abundances_ecoli_df <- as.data.frame(filtered_abundances_ecoli)
  # 
  # rownames(filtered_abundances_ecoli_df) <- paste(filtered_abundances_ecoli_df$PEP.GroupingKey,
  #                                                 filtered_abundances_ecoli_df$PG.ProteinLabel,
  #                                                 filtered_abundances_ecoli_df$species,sep = "@")
  # 
  # filtered_abundances_ecoli_df <- filtered_abundances_ecoli_df %>% select(contains("E2-"))
  # 
  # filtered_abundances_ecoli_imp <- missForest(ntree=5,maxiter = 2,xmis=as.data.frame(filtered_abundances_ecoli_df),verbose = TRUE)
  # 
  # filtered_abundances_ecoli_imp_df <- as.data.frame(filtered_abundances_ecoli_imp[["ximp"]])
  # 
  # filtered_abundances_ecoli_final <- filtered_abundances_ecoli_imp_df %>% 
  #   bind_cols(rownames(filtered_abundances_ecoli_df)) %>%
  #   separate(col = 19,into = c("pep_with_pos","PG.ProteinLabel","species"),sep = "@")
  # 
  ## ADDITIONAL IMPUTATION METHOD with MICE()
  #    library(tidyverse) # DISABLED: all sub-packages already loaded
  library(tidyr)
  library(mice)
  ...
  rownames(filtered_abundances_ecoli) <- paste0(filtered_abundances_ecoli$PEP.GroupingKey,"@",filtered_abundances_ecoli$species,"@",1:nrow(filtered_abundances_ecoli))
  ...
  ## MICE imputation DISABLED (Issue 14 fix) — replaced by Gaussian-modeled P5
  ## The Gaussian P5 imputation was already applied to filtered_abundances above (line ~689),
  ## and abundances_all_aft_imputation was built from it (line ~701).
  ## Keeping the MICE code commented out for reference.
  ...
  # library(VIM)
  #aggr_plot <- aggr(test, col=c('navyblue','red'),
  #numbers=TRUE, sortVars=TRUE,
  #labels=names(intensities), cex.axis=.7,
  #gap=0, ylab=c("Histogram of missing data","Pattern"))
  
  #test <- barplt_df_wide %>% select(exp_design)
  
  #visdat::vis_miss(intensities,cluster = TRUE)
  ...
  # abundances_all_aft_imputation now comes from the Gaussian P5 imputation (line ~701),
  # NOT from MICE. No overwrite needed.
  ...
  write.table(imputed_dataset,file=paste0("imputed_dataset",software_name,".txt"),sep = "\t",row.names = F)
  ...
  p21  <- imputed_dataset %>% filter(grepl(selected_spcies,species.x)) %>% 
    ...
    labs( y= "Count of Intensity", x="log2(Intenisity)",
  ...
  write.table(final_imputed_data, 
              file =paste0(new_path,"/final_imputed_data_Spectronaut_",
  ...
  df_FC_ratio_after_impt <- df_merge %>% 
    select(starts_with("exp_") | contains("species"),Pool,isomericity) %>%
    #separate(accession, into = c("uniprot_id", "species", "position"), remove = F) %>%
    tibble() %>% 
    pivot_longer(cols = starts_with("exp_"),
                 ...
                 values_to = "values")
  #mutate_at("Pool", ~replace_na(.,background_species))
  #df_mean_ab_after_impt <- df_mean_ab_after_impt %>% mutate(species=ifelse(Pool!="ECOLI","HUMAN","ECOLI"))
  ...
  # gg_density(data_set = df_mean_ab_after_impt, 
  #                  x_df = df_mean_ab_after_impt$values,
  ...
  filtered_abundances_ecoli_bef_impt <- barplt_df_ecoli_wide %>% 
    ...
    rename(Pool=species) %>%
    #rename(pep_with_pos=PEP.Grouping.Key) %>%
  ...
  df_before_impt_CV <- tryCatch(CV_calculator_fixed(data = df_merge_before_imp_all,
                                                    exp_id = 2, replicate = num_reps,
                                                    abundance_col = "mean_abundance_A",
                                                    iter = 5,acquisiton_type = acquisiton_type,
                                                    software_name = software_name),
                                error = function(e) { cat("WARNING: CV_calculator (before imputation) failed:", conditionMessage(e), "\n"); NULL })
  
  if (!is.null(df_before_impt_CV)) write.table(df_before_impt_CV,file = paste0(new_path,"/CV_calculation_before_imputation",acquisiton_type,software_name,".txt"))
  ...
  df_aft_impt_CV <- tryCatch(CV_calculator_fixed(data = df_merge_aft_impt,abundance_col = "mean_abundances_aft_imp_A",
                                                 exp_id = 2, replicate = num_reps,
                                                 iter = 5,
                                                 acquisiton_type = acquisiton_type,
                                                 software_name = software_name),
                             error = function(e) { cat("WARNING: CV_calculator (after imputation) failed:", conditionMessage(e), "\n"); NULL })
  
  if (!is.null(df_aft_impt_CV)) write.table(df_aft_impt_CV,file = paste0(new_path,"/CV_calculation_after_imputation",acquisiton_type,software_name,".txt"))
  
  library(ggridges)
  #library(paletteer) # REMOVED: unused 
  
  if (!is.null(df_before_impt_CV) && !is.null(df_aft_impt_CV)) {
    ...
    plot20 <- p20/p21 
  } else { cat("WARNING: CV plots (p18-p21, plot20) skipped due to CV_calculator failure\n") }
  
  ############################## ############################
  #  filtered_abundances_ecoli_bef_impt <- filtered_abundances_ecoli_bef_impt %>% 
  #    mutate(merge=rownames(filtered_abundances_ecoli_bef_impt)) %>%
  #    separate(merge,into = c("pep_with_pos","ProteinLabel","loc_num","species"),sep = "@") %>%
  #    rename(Pool=ProteinLabel) %>%
  #    #rename(pep_with_pos=PEP.GroupingKey) %>%
  #    select(!species) %>% drop_na()
  #  
  #  pep_list_w_theo_iso <- pep_list_w_theo %>% select(pep_with_pos,isomericity)
  #  
  #  df_merge_before_imp <- df_merge_syn %>% left_join(pep_list_w_theo_iso,by="pep_with_pos") %>%
  #    mutate(Pool=ifelse(Pool=="Diluted",paste0(Pool,"_",isomericity),Pool)) %>% 
  #    #pivot_longer(cols = contains("E2"),
  #    #names_to = "Experiment",
  #    #values_to = "Abundances") %>% 
  ...
  #  
  #  df_before_impt_CV <- CV_calculator_fixed(data = df_merge_before_imp_all,
  #                                     exp_id = 2, replicate = num_reps,abundance_col = "mean_abundance_A",iter = 5,acquisiton_type = acquisiton_type,software_name = software_name)
  #  
  #  write.table(df_before_impt_CV,file = paste0(file_path,"CV_calculation_before_imputation",acquisiton_type,software_name,".txt"))
  #  
  #  plot21 <- df_before_impt_CV %>% 
  #    filter(grepl("Diluted",Pool) | grepl("Fixed",Pool)) %>%
  #    ggplot(aes(x=CV_values,y=CV_samples,color=Pool,linetype=Pool)) + 
  # 
  ...
  #    #scale_fill_brewer(palette ="PuOr")+
  #    #scale_fill_grey()+
  #    scale_color_manual(values = c("#FD8D3C","#FD8D3C","#6A51A3","#6A51A3"))+ #"grey68"
  #    scale_linetype_manual(values = c("solid","dashed","solid","dashed")) +
  #    theme_bw() +
  #    #facet_wrap(~Pool) +
  #    theme(aspect.ratio=6.5/11, legend.text = element_text(size = 45),
  #          axis.title.x = element_text(size = 45),
  #          axis.title.y = element_text(size = 45),
  #          plot.title = element_text(size = 55),
  #          legend.title = element_text(size = 45),
  ...
  #  ### AFTER  ### 
  # 
  #  df_merge_aft_impt <- df_merge %>% 
  #    mutate(Pool=ifelse(Pool=="Diluted",paste0(Pool,"_",isomericity),Pool)) %>%
  #    select(!PG.ProteinLabel) 
  #  
  #  df_aft_impt_CV <- CV_calculator_fixed(data = df_merge_aft_impt,abundance_col = "mean_abundances_aft_imp_A",
  #                                  exp_id = 2, replicate = num_reps,
  #                                 iter = 5,
  #                                 acquisiton_type = acquisiton_type,
  #                                 software_name = software_name)
  # write.table(df_aft_impt_CV,file = paste0(file_path,"CV_calculation_after_imputation",acquisiton_type,software_name,".txt"))
  # 
  # df_aft_impt_CV %>% group_by(Pool,CV_samples) %>% summarise(mean_CV_samples=mean(CV_values))
  # 
  # df_aft_impt_CV %>% filter(grepl("Diluted",Pool) | grepl("Fixed",Pool)) %>%
  #   ggplot(aes(y=CV_values,x=Pool,color=CV_samples)) + geom_point() + facet_wrap(~CV_samples)
  # 
  # 
  #  plot18 <- df_aft_impt_CV %>% filter(grepl("Diluted",Pool) | grepl("Fixed",Pool)) %>%
  #    ggplot(aes(x=CV_values,y=CV_samples,color=Pool,linetype=Pool)) + 
  #    
  #    geom_density_ridges(jittered_points = TRUE,
  #                        alpha=0,
  ...
  #    #scale_fill_grey()+
  #    scale_color_manual(values = c("#6A51A3","#6A51A3","#FD8D3C","#FD8D3C"))+ #"grey68"
  #    scale_linetype_manual(values = c("solid","dashed","solid","dashed"))+ #"grey68"
  #            theme_bw() +
  #            #facet_wrap(~Pool) +
  #     theme(aspect.ratio=6.5/11, legend.text = element_text(size = 45),
  #           axis.title.x = element_text(size = 45),
  ...
  #      scale_fill_manual(values =c("#803233FF","#8A4D00FF","#A16928FF","#B0986CFF","#DCBE9BFF"))+ #"#EFDDCFFF",,"#EDEAC2FF"
  #      #scale_fill_grey()+
  #      theme_bw() +
  #      #facet_wrap(~Pool) +
  ...
  #    
  #     
  #  plot20 <-  df_aft_impt_CV %>% filter(!grepl("ECOLI",Pool)) %>% ggplot(aes(x=CV_samples,y=CV_values ,fill=Pool)) + geom_boxplot() + theme_bw() +
  #      #facet_wrap(~Pool) +
  #      theme(legend.text = element_text(size = 45),
  #            axis.title.x = element_text(size = 45),
  #            axis.title.y = element_text(size = 45),
  ...
  if(test_type== "t.test" | test_type== "wilcoxon"){
    ...
    for (i in 2:sample_size) {
      p_values_tmp <- lapply(1:dim(stat_analysis)[1], function(j) {
        if (test_type == "t.test") {
          ttest_func(select(stat_analysis, contains("A",numerator,"-") & contains("log10_"))[j,],
                     select(stat_analysis, contains(paste0("A", i, "-")) & contains("log10_"))[j,])
        } else if (test_type == "wilcoxon") {
          wilcox.test(select(stat_analysis, contains("A",numerator,"-") & contains("log10_"))[j,],
                      select(stat_analysis, contains(paste0("A", i, "-")) & contains("log10_"))[j,])
        }
      })
    ...
    merge_stat_df <- stat_analysis %>%
      ...
      mutate(fold_change_comp = all_pvalues_common_col$fold_change_comp,
      ...
      mutate(new_col_coloring = if_else(grepl("Fixed_nonisomeric", new_col_coloring), "Fixed_nonisomeric", new_col_coloring)) %>%
    ...
    #   assign(paste0("df_A1vsA",i),stat_analysis %>% select(1:2 | contains("A",numerator,"-") & contains("log10_") | contains(paste0("A",i,"-")) & contains("log10_")))
    #   # First, linear model was built
    #   assign(paste0("fit",i) ,lmFit(get(paste0("df_A1vsA",i))[, grep("log10_", colnames(get(paste0("df_A",numerator,"vsA",i)))), drop=FALSE], design_matrix))
  ...
  plot14 <- ggplot(df_roc_func, aes(y=tpr, x = fdp)) +
    geom_path(size=1.5) +
  ...
  dir.create(paste0(file_path,"/outputs_imputed_mice_two_sample_nonNA/"), showWarnings = FALSE)
  sapply(1:length(plt_obj),function(x) ggsave(filename = paste0("p",x,".png"),
                                              width = 90, height = 60, 
                                              path = paste0(file_path,"/outputs_imputed_mice_two_sample_nonNA/"),
                                              units = "cm",
                                              get(plt_obj[x]),
                                              device = "png", #".svg"
  ))
