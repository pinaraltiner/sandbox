...
file_path <- "D:/dev/Pinar/PHD/wet_lab_experiments/DIA_data_analysis/experiment_3/Spectronaut/correct_norm/"
file_name <- "20231018_091803_exp3_correct_norm_Report.tsv" #20230705_125301_OXPAL230127_Exp3_woFAIMS_DIA_Report.tsv"
...
final_spectronaut_pep_quant_analysis_bio <- function(file_path,
  ...
  barplt_df <- quant_phospho_combined %>%
    select(PEP.GroupingKey,Experiment,Intensity,PG.ProteinLabel,species,ptm_score,pep_with_pos) %>% #
  ...
  if(loc_filter_opt == TRUE){
    site_prob <- barplt_df %>% 
      select(ptm_score,Experiment,pep_with_pos) %>% #PEP.GroupingKey
    ...
    barplt_df_wide <- barplt_df %>%  ## If you select "charge" column, it will bring multiple rows for one seq
      select(PEP.GroupingKey,Experiment,Intensity,PG.ProteinLabel,species,pep_with_pos) %>%
      pivot_wider(names_from = "Experiment",values_from = "Intensity") %>%
      left_join(site_prob, by="pep_with_pos") %>%
      #mutate(species=selected_spcies) %>%
      filter(max_value >= loc_filter) %>%
      relocate(exp_design,.after = "PG.ProteinLabel") %>%
      relocate(pep_with_pos,.before = "PEP.GroupingKey") %>%
  ...
  barplt_df_ecoli_wide <- barplt_df_ecoli %>%
    select(pep_with_pos,Experiment,Intensity,PG.ProteinLabel,species)%>%
    pivot_wider(names_from = "Experiment",values_from = "Intensity") %>%
    #mutate(species=background_species) %>% 
    relocate(exp_design,.after = "PG.ProteinLabel")
  ...
  library(kableExtra)
  na_phospho_selected <- apply(X = is.na(filtered_abundances %>% select(PEP.GroupingKey, PG.ProteinLabel,starts_with(paste0("E",exp_id)))), MARGIN = 2, FUN = sum)
  na_ecoli <- apply(X = is.na(filtered_abundances_ecoli %>% select(PEP.GroupingKey,PG.ProteinLabel,starts_with(paste0("E",exp_id)))), MARGIN = 2, FUN = sum)
  ...
  abundances_all_aft_imputation <- bind_rows(filtered_abundances, filtered_abundances_ecoli)
  ...
  for (k in 1:sample_size){
    ...
    assign(paste0("aft_imp_abundances_A",k),as.data.frame(rowMeans(abundances_all_aft_imputation  %>% 
                                                                     select(contains(paste0("A",k))))))
    
    filtered_abundances_rowMeans<- bind_cols(filtered_abundances_rowMeans,get(paste0("aft_imp_abundances_A",k)))
    
    assign(paste0("log10_abundances_A",k),as.data.frame(log10(abundances_all_aft_imputation  %>%
                                                                select(contains(paste0("A",k))))))
    
    filtered_abundances_log10 <- bind_cols(filtered_abundances_log10, get(paste0("log10_abundances_A",k)))
    
    assign(paste0("rowMean_log10_abundances_A",k),as.data.frame(rowMeans(get(paste0("log10_abundances_A",k)) %>% 
                                                                           select(contains(paste0("A",k))))))
  ...
  #final_imputed_data_mouse <- final_imputed_data %>% 
    #filter(grepl(selected_spcies,species))
  
  #final_imputed_data_ecoli <- final_imputed_data  %>%
    #filter(!grepl(selected_spcies, species))
  
  
  df_mean_ab_after_impt <- final_imputed_data %>% 
    select(contains("aft_imp") | contains("species")) %>%
    tibble() %>% 
  ...
  df_FC_ratio_after_impt <- final_imputed_data %>% 
    select(starts_with("exp_") | contains("species")) %>%
    #separate(accession, into = c("uniprot_id", "species", "position"), remove = F) %>%
    tibble() %>% 
  ...
  wilcox_func <- function(x, y) {
    ...
    #   
    # }
    wilcox.test(x, y,alternative = c("two.sided"))$p.value
    
    
  }
  ## DATA FOR STATISTICAL ANALYSIS
  stat_analysis <- final_imputed_data %>%
    select(pep_with_pos,species,max_value, starts_with("log10_") | starts_with("mean_log10_") | starts_with("exp_FC")) %>% #pep_with_pos, Pool, isomericity
    filter(!grepl(background_species, species))
  #filter(grepl("HUMAN",accession)) #spectrum_title
  
  rownames(stat_analysis) <- paste0(stat_analysis$pep_with_pos,"@",stat_analysis$max_value,"@",(1:nrow(stat_analysis))) #stat_analysis$spectrum_title,"@"
  library(multtest)
  if(test_type== "t.test" | test_type== "wilcoxon"){
    ...
    for (i in 2:sample_size) {
      p_values_tmp <- lapply(1:dim(stat_analysis)[1], function(j) {
        if (test_type == "t.test") {
          ...
          wilcox.test(as.numeric(select(stat_analysis, contains("A1-") & contains("log10_"))[j,]),
                      as.numeric(select(stat_analysis, contains(paste0("A", i, "-")) & contains("log10_"))[j,]))$p.value
        }
      })
    ...
    all_pvalues_common_col <- stat_analysis %>% 
      select(pep_with_pos, max_value) %>% #spectrum_title
      ...
      select(!c(tmp,tmp1)) %>%
      mutate(common_col = paste(pep_with_pos,max_value,A1vs_Ai,1:((sample_size-1)*nrow(stat_analysis)),sep="@")) #spectrum_title
    
    ## THE BEST WAY TO DO is this:
    merge_stat_df <- stat_analysis %>%
      select(pep_with_pos, max_value, starts_with("exp_FC")) %>%
      ...
      rename_with(.col =5 , ~"A1vs_Ai") %>%
      rename_with(.col=6, ~ "P.Value")# %>%
    ...
    for ( i in 2:sample_size){
      ...
      assign(paste0("df_A1vsA",i),stat_analysis %>% select(1:3 | contains("A1-") & contains("log10_") | contains(paste0("A",i,"-")) & contains("log10_")))
      # First, linear model was built
      assign(paste0("fit",i) ,lmFit(get(paste0("df_A1vsA",i))[,4:9], design_matrix))
    ...
    merge_stat_df1 <- merge_stat_df %>% 
      separate(common_col, into = c("pep_with_pos","max_values","tmp"),sep = "@") %>%
      select(!tmp) %>%
      rename_with(.col=9, ~ "A1vs_Ai") %>%
      separate(A1vs_Ai, into = c("first","second"),sep = "-") %>%
      mutate(A1vs_Ai = paste(first,second,sep = "/")) %>%
      mutate(common_col = paste(pep_with_pos,max_values,A1vs_Ai,sep = "@")) %>%
    ...
    merge_stat_df_final <- stat_analysis %>%
      ...
      mutate(common_col = paste(pep_with_pos,max_value,ratio,sep = "@")) %>%
     ...
     #mutate(new_col_coloring = if_else(grepl("ISO-REF", new_col_coloring), "ISO-REF", new_col_coloring)) %>%
      #mutate(new_col_coloring = if_else(grepl("unexpected", new_col_coloring), "unexpected", new_col_coloring)) %>%
      rename(pep_with_pos=pep_with_pos.x)# %>% rename(max_value=max_value.x)
  ...
  point_count_y_axis <- merge_stat_df_final %>%
    group_by(A1vs_Ai) %>%
    filter(P.Value < 0.05) %>% 
    count(A1vs_Ai) %>% left_join(actual_ratio_col)
  ...
  p9 <- ggplot(merge_stat_df_final,aes(x =log2(merge_stat_df_final$fold_change_values), y = -log10(merge_stat_df_final$P.Value))) +
    geom_point(aes(color = A1vs_Ai), size = 1) + #,shape=Pool_new
    ...
    labs( y= "-log10(p values)", x="log2(fold change)",title = paste("Experiment - ", exp_id, acquisiton_type, " data processed by ", software_name),
          subtitle = paste(test_type, "was used \n", subtitle)) +
    ...
    geom_label(data = point_count_y_axis, aes(x = log2(actual_ratio_val), y = y_pos, label = n),size=14, fill=c("#CC79A7","#E69F00","#56B4E9","#009E73"), colour="white",show.legend = FALSE) 
