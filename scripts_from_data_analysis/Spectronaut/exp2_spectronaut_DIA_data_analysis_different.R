...
final_spectronaut_pep_quant_analysis_syn <- function(file_path,
  ...
  intended_dir <-paste0(file_path,"output_final_aft_mapp_func/") #final_res_maxfilt #1diffLocFilt_5thpercent1nonNAdiffmap_Charge14
  ...
  exp_design <- str_sort(exp_design)
    ...
    find_max_value_and_pos <- function(ptm_count, ptm_prob, ptm_pos) {
      ...
      if (dim_count >= 2) {
        ...
        sorted_sel_pos_val <- order(sorted_pos_values[1:length(highest_dim)],decreasing = FALSE)
        #for (i in 1:length(highest_dim)){
        result["prob"] <-max(sorted_prob_values) #highest_dim[sorted_sel_pos_val] # #paste(sorted_prob_values[1:length(highest_dim)],collapse ="&")
        result["pos"] <- paste(sorted_pos_values[sorted_sel_pos_val],collapse = "&")#paste(sorted_pos_values[1:length(highest_dim)],collapse ="&")
    ...
    quant_peptides_with_cond <- quant_peptides %>% 
      ...
      separate(EG.PrecursorId,into = c("Precursor_pep","Charge"),sep = "\\.", remove = F) %>%
    ...
    imputed_values <- quant_peptides_with_cond  %>%
        group_by(Experiment) %>% 
        summarise(first_quantile=quantile(Intensity,probs=0.05,na.rm=TRUE)) ## changed from 1 to 5
    ...
    pep_list_w_theo_seq_map <- pep_list_w_theo %>% select(Sequence,Neutral.mass.SH.Cys) %>%
      rename(PEP.GroupingKey = Sequence) %>%
    ...
    ## PHOSPHO-FILTERING
    quant_phospho <- quant_peptides_with_cond %>% filter(grepl("HUMAN", PG.ProteinLabel) & !grepl('CON__',PG.ProteinLabel)) %>%
    ...
    barplt_df_new <- quant_phospho_combined %>%
    ...
    barplt_df_new$Sum <- mapply(calculate_sum, barplt_df_new$PEP.GroupingKey, barplt_df_new$Positions, MoreArgs = list(mapping_df = mapping_df))
    
    barplt_df_mapped_seq <- barplt_df_new %>% 
      mutate(Neutral_mass = round(Sum, 0)) %>%
      mutate(Neutral_mass_res=Neutral_mass) %>%
      full_join(pep_list_w_theo_seq_map,by=c("Neutral_mass","PEP.GroupingKey")) %>% # #pep_with_pos
    ...
    barplt_df_mapped_pep <- barplt_df_new %>% 
      mutate(Neutral_mass = round(Sum, 0)) %>%
      mutate(Neutral_mass_res=Neutral_mass) %>%
      full_join(pep_list_w_theo_seq_map,by=c("Neutral_mass","PEP.GroupingKey")) %>% # #pep_with_pos
    ...
    site_prob <- barplt_df_mapped_pep %>%
      ...
      rowwise() %>%
      mutate(max_value = max(c_across(all_of(exp_design)), na.rm = TRUE)) %>%
      #mutate(mean_value = mean(as.numeric(c_across(all_of(exp_design))), na.rm = TRUE)) %>%
      select(!exp_design)
    ...
    write.table(barplt_df_wide_all_col,file =  paste0(new_path,"/Count_of_missing_unexpected_correct_phospho-sites_with_all_col_",
                                                      software_name,"_Experiment",exp_id,".txt"), sep = "\t",row.names = F)
    ...
    all_seq <- barplt_df_mapped_seq %>% 
      ...
      mutate(species=is.na(species),selected_spcies,species)
    ...
    plot15 <- gg_barplt_id_pep_count(data_set = all_seq_syn,
                                     ...
                                     ymax =length(unique(all_seq_syn$PEP.GroupingKey)) ,
                                     size_num=10,
    ...
    all_pep_syn <- barplt_df_mapped_pep %>%
      #filter(!grepl(background_species,Pool_for_seq_merge)) %>%
      select(pep_with_pos, EG.PrecursorId, map_loc,ptm_score,Experiment,Sample_id,Rep_id) %>%
    ...
    plot12 <- gg_barplt_id_pep_count(data_set = all_corr_pep_syn,
                                     ...
                                     size_num = 10,
                                     header = paste("Total number of identified & localized phosphorylated", selected_spcies,"across each sample",sep=" "),
    ...
    plot1 <- gg_barplt_id_pep_count(data_set = barplt_df_new,
                                 x_df = barplt_df_new$Sample_id,
                                 fill_df = barplt_df_new$Rep_id,
                                 ymax = 20000,
                                 size_num = 10,
    ...
    #   select(!exp_design)
    # 
    # barplt_df_wide_all_col <- barplt_df_new%>%
    ...
    if(loc_filter_opt == TRUE){
      ...
      #   select(EG.PrecursorId,Experiment,Intensity,pep_with_pos,ptm_score,PG.ProteinLabel) %>% #PEP.GroupingKey
      # 
      #   #left_join(site_prob, by="pep_with_pos") %>%
      ...
      barplt_df_wide <- barplt_df_mapped_pep %>%  ## If you select "charge" column, it will bring multiple rows for one seq
        ...
        left_join(site_prob, by="pep_with_pos") %>%
        mutate(species=selected_spcies) %>%
        #filter(mean_value >= loc_filter) %>%
        relocate(pep_with_pos, .after = "PEP.GroupingKey") 
    }
    ######################## MERGING WITH FILTERED LOC SCORES INTENSITIES INTO ONE ROW ############################
    #value_columns <- names(barplt_df_wide)[grepl("-R", names(barplt_df_wide))][order(names(barplt_df_wide)[grepl("-R", names(barplt_df_wide))], decreasing = FALSE)]
    ...
    #   ) %>%
    #   mutate(ptm_score_num=as.numeric(ptm_score)) %>%
   ...
   # filtered_abundances_ecoli <- barplt_df_ecoli_wide
    ...
    df_merge_syn <- filtered_abundances %>%
      ...
      mutate(Pool= ifelse(is.na (species),"missing",Pool)) %>% #PG.ProteinLabel
      mutate(Pool=ifelse(Pool=="Diluted",paste0(Pool,'_',isomericity),Pool)) %>%
    ...
    barplt_phospho_seq <- filtered_abundances %>%
      ...
      separate(pep_with_pos,into = c("PEP.GroupingKey","phospho_pos"),sep = "_") %>%
    ...
    ### IMPUTATION ONLY 2NAs per sample or 3NAs per sample ###
    # filtered_abundances <- as.data.frame(filtered_abundances)
    # rownames(filtered_abundances) <- paste0(filtered_abundances$pep_with_pos,"@",filtered_abundances$species,"@",1:nrow(filtered_abundances))
    # filtered_abundances <- filtered_abundances %>% bind_cols(1:nrow(filtered_abundances)) 
    # colnames(filtered_abundances)[23] <- "indx"
    # 
    # 
    # filtered_abundances_ecoli <- as.data.frame(filtered_abundances_ecoli)
    # rownames(filtered_abundances_ecoli) <- paste0(filtered_abundances_ecoli$PEP.GroupingKey,"@",filtered_abundances_ecoli$species,"@",1:nrow(filtered_abundances_ecoli))
    # 
    # filtered_abundances_ecoli <- filtered_abundances_ecoli %>% bind_cols(1:nrow(filtered_abundances_ecoli)) 
    # colnames(filtered_abundances_ecoli)[22] <- "indx"
    #
  
    # filtered_abundances <- conditional_imputation(df=filtered_abundances,samp_names = sample_names,num_NA_imputed = 2,impute_val = imputed_values_vec)
    # filtered_abundances_ecoli <- conditional_imputation(df=filtered_abundances_ecoli,samp_names = sample_names,num_NA_imputed = 2,impute_val = imputed_values_vec)
    # 
    # # filtered_abundances_rowname <- filtered_abundances %>%
    # #   mutate(tmp=rownames(filtered_abundances))
    # # 
    # # abundances_all_aft_imputation <- filtered_abundances_ecoli %>%
    # #   mutate(tmp=rownames(filtered_abundances_ecoli)) %>%
    # #   bind_rows(filtered_abundances_rowname) %>%
    # #   separate(tmp,into = c("pep_with_pos","species","indx"),sep = "@") %>%
    # #   select(!indx)
    # # 
    # 
    #  
    # ### IMPUTATION 5th percentile 2NAs per sample or 3NAs per sample & mice for 1NA per sample ###
    # new_experiment_name <-c("E2_A1_R1",
    #                         "E2_A1_R2",
    #                         "E2_A1_R3",
    #                         "E2_A2_R1",
    ...
    #                         "E2_A6_R3")
    # 
    # colnames(filtered_abundances) <- new_experiment_name
    # colnames(filtered_abundances_ecoli) <- new_experiment_name
    # 
    # library(mice)
    # imp_intensities <-  mice(filtered_abundances,m=5,maxit=10,meth='cart',seed=500)
    # imp_intensitiesECOLI <- mice(filtered_abundances_ecoli,m=5,maxit=1,meth='cart',seed=500) #maxit=10
    # 
    # completeData <- complete(imp_intensities,2)
    # colnames(completeData) <- exp_design
    # 
    ...
    final_imputed_data_syn <- final_imputed_data %>% mutate(species=ifelse(is.na(species), selected_spcies,species)) %>%
  ...
  # #scale_y_continuous(limits = c(0,200)) + #breaks = seq(from =0, to=100,by=10)) 
    ...
    df_merge_before_imp <- df_merge_syn %>% #filtered_abundances_bfr_impt
     ...
     select(!isomericity) %>%
      ...
      filter(!grepl("missing",Pool) & !grepl("Unexpected",Pool))
    ...
    df_before_impt_CV <- CV_calculator(data = df_merge_before_imp_all,
                                       ...
                                       iter = 5,replicate = num_reps,acquisiton_type = acquisiton_type,
    ...
    p18<- df_before_impt_CV %>% filter(grepl("Diluted",Pool) | grepl("Fixed",Pool)) %>%
      ...
      xlim(0,round(max(df_before_impt_CV$CV_values))) #xlim(0,100)
    # scale_y_continuous(
    # limits = c(0,100),
    # breaks = seq(0, 100,10))
    ...
    p19 <- df_aft_impt_CV %>% filter(grepl("Diluted",Pool) | grepl("Fixed",Pool)) %>%
      ...
      xlim(0,round(max(df_aft_impt_CV$CV_values)))
    #scale_y_continuous(
   ...
   #  
   #  df_before_impt_CV <- CV_calculator(data = df_merge_before_imp_all,abundance_col = "mean_abundance_A",iter = 5,acquisiton_type = acquisiton_type,software_name = software_name)
   #  
   ...
   #      )
    ...
    write.table(merge_stat_df_final_text,file = paste0(new_path,"/1volcano_plot_",software_name,"_",acquisiton_type,".txt"),sep = 
                  "\t",col.names = T,row.names = F)
    ...
    sapply(1:length(plt_obj),function(x) ggsave(filename = paste0("p",x,".png"),
                                                 width = 90, height = 60, 
                                                 path = paste0(file_path,"/output_final_aft_mapp_func"), #final_res_maxfilt  ##1diffLocFilt_5thpercent1nonNAdiffmap_Charge14/" #/outputs_imputed_5thpercent_one_sample_nonNA/"),
                                                 units = "cm",
                                                 get(plt_obj[x]),
                                                 device = "png", #".svg"
    ))
