...
final_proline_pep_quant_analysis <- function(file_path,
  ...
  phospho_ptm_pos <- lapply(syn_phospho_pep$ptm_protein_positions, function(each_ptm_protein_positions) {
    ptm_list <-
      as.list(strsplit(each_ptm_protein_positions, "; ", fixed = TRUE)[[1]]) # Split ptm_protein_position depending on ";"
    ptm_list <-
      ptm_list[grepl("Phospho", ptm_list, fixed = TRUE)] # Extract only which contains "Phospho"
    ...
    phospho_positions_as_str <-
      paste(phospho_positions, collapse = "&") #combine each position with "|"
  ...
  common_col_theo_quant <- as.data.frame(
    paste(
      pep_list_w_theo_quant$Phospopeptide.sequence,
      pep_list_w_theo_quant$modified.position.in.peptide,
      sep = "_"
    )
  )
  ...
  for (i in 1:length(sorted_colnames)) {
    assign(paste0("tmp", i), sort(colnames(select(
      df_merge,
  ...
  for (k in 1:sample_size){
    ...
    assign(paste0("abundances_A", k), as.data.frame(rowMeans(
      abundances_only_for_impute  %>% select(contains(paste0("A", k))) %>%
        select(starts_with("abundance_"))
    )))
    filtered_abundances_rowMeans <-
      bind_cols(filtered_abundances_rowMeans, get(paste0("abundances_A", k)))
  ...
  for(An in 2:cols) {
    filtered_abundances_rowMeans[, paste0("exp_FC_A1/A", An)] <-
      filtered_abundances_rowMeans[, 1] / filtered_abundances_rowMeans[, An]
  ...
  df_for_figure <- final_imputed_normalized_data %>%
    ...
    pivot_longer(
      cols = col_sel_stat_analysis[3:11],
      names_to = "Theo_Exp",
      values_to = "Ratios",
      values_drop_na = T
    ) %>%
  ...
  figure_with_mean_abundance <- final_imputed_normalized_data %>%
    select(starts_with("mean_") |
             contains(c("Pool", "common_col_for_merging"))) %>%
    pivot_longer(
