...
final_proline_pep_quant_analysis <- function(file_path,
  ...
  df_merge <- syn_phospho_pep_proline_new %>% 
    left_join(pep_list_w_theo_quant_new,by="common_col_for_merging")
  
  ## ALWAYS KEEP IT COMMENTED TO PREVENT OVERWRITE
  #write.xlsx(df_merge,file = "D:/dev/Desktop_copy/PHD/wet_lab_experiments/experiment2_quantification_peptide_level/merging_exp_syn_phospho_pep_proline_and_pep_list_theo_quant_exp2_before_stast_analysis_08012023.xlsx")
  ...
  ### IMPUTATION
  abundances_for_impute <- filtered_abundances %>% 
  ...
  syn_phospho_pep_proline_unique_max_absum <- filtered_abundances %>% 
    select(-contains("abundance")) %>% mutate(abundances_for_impute) %>%
  ...
  for (k in 1:sample_size){
    ...
    assign(paste0("row_means_abundances_A",k),as.data.frame(rowMeans(syn_phospho_pep_proline_unique_max_absum %>% select(contains(paste0("A",k))) %>%
                                                             select(starts_with("abundance_")))))
    
    assign(paste0("log10_abundances_A",k),as.data.frame(log10(syn_phospho_pep_proline_unique_max_absum  %>% select(contains(paste0("A",k))) %>%
                                                             select(starts_with("abundance_")))))
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  for (i in 12:15){
     assign(paste0("complete_p_values_",i), 
             final_imputed_data %>% 
               ...
               mutate_at(c('Pool'), ~replace_na(.,"unexpected")))
  ...
  roc_data12 <- cbind(roc_data12, "A1 vs A2","Others")
  roc_data13 <- cbind(roc_data13, "A1 vs A3","Others")
  roc_data14 <- cbind(roc_data14, "A1 vs A4","Others")
  roc_data15 <- cbind(roc_data15, "A1 vs A5","Others")
  
  colnames(roc_data12)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data13)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data14)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data15)[c(4,5)] <- c("Comparison","Pool")
  
  roc_data_unexpected12 <- cbind(roc_data_unexpected12, "A1 vs A2","unexcepted")
  roc_data_unexpected13 <- cbind(roc_data_unexpected13, "A1 vs A3","unexcepted")
  roc_data_unexpected14 <- cbind(roc_data_unexpected14, "A1 vs A4","unexcepted")
  roc_data_unexpected15 <- cbind(roc_data_unexpected15, "A1 vs A5","unexcepted")
  
  colnames(roc_data_unexpected12)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data_unexpected13)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data_unexpected14)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data_unexpected15)[c(4,5)] <- c("Comparison","Pool")

  
  
  roc_final <- rbind(roc_data12,roc_data13,roc_data14,roc_data15,
                     roc_data_unexpected12,roc_data_unexpected13,
                     roc_data_unexpected14,roc_data_unexpected15)
