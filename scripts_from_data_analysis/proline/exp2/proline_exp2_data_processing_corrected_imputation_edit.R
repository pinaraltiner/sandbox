...
final_proline_pep_quant_analysis <- function(file_path,
  ...
  df_merge <- syn_phospho_pep_proline_unique_max_absum %>% 
  ...
  ##TODO put this into another for loop to reduce redundancy of the code  
    for(j in 1:dim(stat_analysis)[1]){
      ...
      #(sum(stat_analysis[j,8:10])/3) !=  stat_analysis[j,8:10][1] |
      #(sum(stat_analysis[j,11:13])/3) !=  stat_analysis[j,11:13][1] |
       # (sum(stat_analysis[j,14:16])/3) !=  stat_analysis[j,14:16][1]
      }
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  p_thresholds<- c(2.485779e-02,3.327239e-02,3.219350e-02,3.601032e-02)
