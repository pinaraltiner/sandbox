...
final_proline_pep_quant_analysis <- function(file_path,
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  roc_data121 <- cbind(roc_data12, "A1 vs A2","Others")
  roc_data131 <- cbind(roc_data13, "A1 vs A3","Others")
  roc_data141 <- cbind(roc_data14, "A1 vs A4","Others")
  roc_data151 <- cbind(roc_data15, "A1 vs A5","Others")
  
  colnames(roc_data121)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data131)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data141)[c(4,5)] <- c("Comparison","Pool")
  colnames(roc_data151)[c(4,5)] <- c("Comparison","Pool")
  ...
  ggplot(roc_final2,aes(x=fdp,y=tpr,color=Comparison,linetype=Pool)) + geom_line(size=1) +
    theme_bw() +
