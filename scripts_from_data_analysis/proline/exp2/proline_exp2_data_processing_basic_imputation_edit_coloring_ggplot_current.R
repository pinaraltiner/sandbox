...
final_proline_pep_quant_analysis <- function(file_path,
  ...
  ### IMPUTATION
  abundances_for_impute <- exp2_noFAIMS_DDA_proline %>% 
  ...
  selected_cols <- colnames(exp2_noFAIMS_DDA_proline[c(1:4,6,28)])
  ...
  write.table(final_imputed_data,file="D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/Proline_data_analysis/with_FAIMS/exp2_without_FAIMS_Proline_after_merging_psm_count.tsv",sep = "\t")
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  p_thresholds<- c(1.969200e-02,2.359024e-02,2.751729e-02,2.887094e-02)
  ...
  roc_data121 <- cbind(roc_data12, "A1 vs A2","Others", "Proline_no_faims")
  roc_data131 <- cbind(roc_data13, "A1 vs A3","Others", "Proline_no_faims")
  roc_data141 <- cbind(roc_data14, "A1 vs A4","Others", "Proline_no_faims")
  roc_data151 <- cbind(roc_data15, "A1 vs A5","Others", "Proline_no_faims")
  ...
  roc_curve_data_exp2_FAIMS_proline <- read.delim("D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/Proline_data_analysis/with_FAIMS/roc_curve_data.tsv",sep = "\t")
  
  
  roc_proline_mq <- rbind(roc_curve_data_exp2_FAIMS_proline,roc_curve_data_exp2_noFAIMS_MQ,roc_final)
  ...
  ggplot(roc_proline_mq,aes(x=fdp,y=tpr)) + 
    geom_line(size=1,aes(color=Comparison,linetype=Software)) +
