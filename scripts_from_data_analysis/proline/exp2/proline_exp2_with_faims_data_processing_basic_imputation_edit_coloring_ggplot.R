...
final_proline_pep_quant_analysis <- function(file_path,
  ...
  selected_cols <- colnames(exp2_FAIMS_DDA_proline[c(1:4,28)])
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  p_thresholds<- c(0.0166820406,2.149106e-02,2.019552e-02,2.067138e-02)
  ...
  write.table(roc_final,file = "D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/Proline_data_analysis/with_FAIMS/roc_curve_data.tsv",sep = "\t",row.names = F)
  roc_curve_datapro_no_faims <- read.delim("D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/Proline_data_analysis/exp2_re_injection/exp2wo_FAIMS_DDA_Proline_volcano_plot_data.tsv",sep = "\t")
  ...
  ggplot(roc_final,aes(x=fdp,y=tpr)) + 
    ...
    labs(title = "Experiment 2 - DDA with FAIMS - \n Comparision of Softwares")
  ...
  ggplot(volcano_final1, aes(x = log2(exp_FC_value), y = -log10(pvalues_value))) +
    ...
    labs(title = "Experiment 2 - DDA with FAIMS processed by Proline", color = "Classes", shape="Type")
