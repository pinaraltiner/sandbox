...
#################################################  
   ...
   # new_experiment_name <-c("E2_A1_R1",
    ...
    write.table(final_imputed_data, 
                file =paste0(new_path,"/final_imputed_data_Proline_",
...
######### COLORING STRATEGY BASED ON ONLY RATIO and SAMPLE NAME (coloring_comp) #########
    ...
    theme_bw() +
      ...
      theme(legend.text = element_text(size = 45),
            ...
            axis.text.y = element_text(size = 45),
            plot.subtitle = element_text(size = 45)) +
      labs( y= "-log10(p values)", x="log2(fold change)",title = paste("Experiment - ", exp_id, acquisiton_type, " data processed by ", software_name), subtitle = paste("5th percentile imputation for at least 2 NAs and \n mice imputation for 1NA per one sample after filtering nonNa of 1 Sample \n",
