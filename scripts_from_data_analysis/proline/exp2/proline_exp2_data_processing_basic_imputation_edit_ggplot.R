...
final_proline_pep_quant_analysis <- function(file_path,
 ...
 ### TODO: Extract where the first FALSE was generated to use for threshold of each comparison.
  ...
  ggplot(roc_final,aes(x=fdp,y=tpr)) + 
    geom_line(size=1,aes(color=Comparison)) +
  ...
  ggplot(volcano_final1,aes(x=log2(exp_FC_value),y=-log10(pvalues_value))) +
    geom_point(aes(shape=Pool_new, color=Pool_new),size=2.5) + 
    scale_color_manual(values = c("#CC79A7", "#E69F00", "#56B4E9", "#009E73","#000000", "#D55E00","darkgoldenrod","#0072B2"),
                       labels=c('A1 vs A2','A1 vs A3','A1 vs A4','A1 vs A5','Non-variant', 'Variant isomeric', 'Variant non-isomeric','Unexpected')) +
    scale_shape_manual(values = c(16,15,12,17),
                       labels = c('Non-variant', 'Variant isomeric', 'Variant non-isomeric','Unexpected'))+
    ###### CHANGE NA to UNEXPECTED to MAKE THEM VISIBLE
    #geom_label( 
   ...
   # ) +
    ...
    scale_fill_discrete(labels=c('Non-variant', 'Variant isomeric', 'Variant non-isomeric','Unexpected')) +
    theme_bw() +
    theme(legend.text = element_text(size=15), 
          axis.title.x = element_text(size = 15),
          axis.title.y = element_text(size = 15),
    ...
    expand_limits(x = 0, y = 0) +
    #facet_wrap(~Ratio_col,scales = "free_x") +
    geom_vline(data=actual_ratio, aes(xintercept =actual_ratio$log2.c.2..10..20..100..,color=Ratio_col), size=1,show.legend = FALSE) + 
    geom_hline(data=log10_p_thresholds, aes(yintercept=log10_p_thresholds$X.log10.p_thresholds.,color=ratio_col),size=1,linetype=2,show.legend = FALSE)+
