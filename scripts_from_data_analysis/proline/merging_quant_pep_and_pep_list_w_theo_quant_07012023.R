...
# Nothing is changed
filtered_abundances<-abundances[rowSums(!is.na(select(df_merge_stat_analysis,starts_with("abundance_"))))>0,]
...
for (j in 1:length(impute_values)){
  # Number NA 
  num_NA <- length(filtered_abundances[,j][is.na(filtered_abundances[,j])])
  
  filtered_abundances[,j][is.na(filtered_abundances[,j])] <- impute_values[j]
  
  # After imputation number of imputed values
  num_imp <-length(filtered_abundances[,j][(filtered_abundances[,j]==impute_values[j])])
...
# take log10 
log_10_filtered_abundances <- log10(filtered_abundances[,-c(1:4)])
colnames(log_10_filtered_abundances) <- paste0(colnames(filtered_abundances[,-c(1:4)]),"_log10")
final_imputed_normalized_data <- cbind(filtered_abundances,log_10_filtered_abundances)
