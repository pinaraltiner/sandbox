...
sample_size <- 5
log_10_filtered_abundances_rowMeans <- NULL
for (k in 1:sample_size){
  ...
  assign(paste0("abundances_log10_A",k),as.data.frame(rowMeans(log_10_abundances_only_for_impute  %>% select(contains(paste0("A",k))) %>%
                                                                 select(starts_with("abundance_")))))
  log_10_filtered_abundances_rowMeans<- bind_cols(log_10_filtered_abundances_rowMeans,get(paste0("abundances_log10_A",k)))
  
}

colnames(log_10_filtered_abundances_rowMeans) <- paste0("mean_abundances_log10_A",1:sample_size)

# Calculate Fold Change by keeping A1 constant (mean(S1)/mean(S2), etc.)
cols <- ncol(log_10_filtered_abundances_rowMeans)
for(An in 2:cols){
  log_10_filtered_abundances_rowMeans[,paste0("exp_FC_A1/A",An)] <- log_10_filtered_abundances_rowMeans[,1]/log_10_filtered_abundances_rowMeans[,An]
  
}
# To calculate all binary combination in the data frame
#mat <- do.call(cbind, lapply(cols, function(xj) 
#  sapply(cols, function(xi) (log_10_filtered_abundances_rowMeans[, xj]/(log_10_filtered_abundances_rowMeans[, xj])))))
#colnames(mat) <-  outer(names(log_10_filtered_abundances_rowMeans), names(log_10_filtered_abundances_rowMeans), paste0)

final_imputed_normalized_data <- cbind(filtered_abundances,log_10_filtered_abundances_rowMeans)
...
p_values_A1_vs_A5 <- NULL
for(i in 1:nrow(log_10_abundances_only_for_impute)){
  t_test_res <- t.test(log_10_abundances_only_for_impute[i,1:3],log_10_abundances_only_for_impute[i,13:15], alternative = "two.sided", var.equal = TRUE)
  p_values_A1_vs_A5[i] <- t_test_res$p.value
}
