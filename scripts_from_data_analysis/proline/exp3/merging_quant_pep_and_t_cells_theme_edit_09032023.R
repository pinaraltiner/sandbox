...
quant_peptides <- read.xlsx("D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_3/Proline_data_analysis_06022023/PAL _T_cell_Exp3_( 5 conc 3reps)_NoFAIMS_DDA_with_cont_230206_2023-02-07_0947.xlsx", sheet = "Quantified peptide ions")
...
ggplot(data = melt_abundances_rowMeans, aes(x= log10(value))) + 
...
melt_filtered_abundances_rowMeans_FC <- melt(final_imputed_normalized_data[,116:119])
...
ggplot(data = melt_filtered_abundances_rowMeans, aes(log2(value),color=variable)) + geom_density(aes(color=variable),size=1) +
  #geom_violin(colour = "black",size = 0.8) + #bw(0.6)  + #geom_point() +
  theme_bw() +
  theme(legend.text = element_text(size=15), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
        axis.title.x = element_text(size = 15),axis.title.y = element_text(size = 15),
