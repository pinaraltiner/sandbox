...
quant_peptides <- read.xlsx("D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_2/Proline_data_analysis/PAL - 2023 01 04 - Phosphopeptides exp2 ( 5 conc 3 reps) DDA_2023-01-06_1032.xlsx", sheet = "Quantified peptide ions")
...
pep_list_w_theo_quant <- read.xlsx("D:/dev/Desktop_copy/PHD/wet_lab_experiments/Eyers_syn_peptides_experiment/Synthetic peptides list_theo_conc_added_080122.xlsx", sheet = "ISO-ref and OTHER with FC")
...
ggplot(df_for_figure,aes(x=log10(Ratios), fill=Theo_Exp )) +
  stat_density(position = "stack",alpha=0.4, aes(color=Pool),size=1) + 
  ...
  ) + ggtitle("Distribution of experimental quantitative ratios with two different pools") +
  facet_wrap(~ Theo_Exp, nrow = 2)
...
figure_with_mean_abundance <- final_imputed_normalized_data %>%
  ...
  ggplot(figure_with_mean_abundance,aes(x=log10(Mean_abundance), fill=Theo_Exp )) +
  stat_density(position = "identity",alpha=0.4, aes(color=Pool),size=1) + 
  #stat_density(position = "identity") 
  #https://ggplot2.tidyverse.org/reference/geom_density.html
  scale_fill_brewer(palette = 2,direction=-1) +
  scale_linetype_manual(values=c("dashed", "dotted")) +
  ...
  ggtitle("Distribution of mean abundance of every sample with two different pools")
...
ggplot(df_for_figure,aes(x =Theo_Exp , y =log2(Ratios),fill = Pool)  ) +
    geom_boxplot() +
    theme_minimal() +
    theme(legend.text = element_text(size=15), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
          axis.title.x = element_text(size = 15),axis.title.y = element_text(size = 15),
...
ggplot(df_for_figure,aes(x =Theo_Exp , y =log2(Ratios),fill = Pool)) +
  geom_half_boxplot(outlier.shape = NA,aes(color=Pool)) +
  geom_half_point(alpha = 0.5, show.legend = FALSE, aes(color=Pool))+
  scale_y_continuous(breaks = c(-5:15,3)) +
  theme_minimal() +
  theme(legend.text = element_text(size=15), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
        axis.title.x = element_text(size = 15),axis.title.y = element_text(size = 15),
        plot.title = element_text(size=20),
...
ggplot(df_for_figure,aes(x =Theo_Exp , y =log2(Ratios),fill = Pool)) +
  geom_violin(trim = FALSE,aes(color=Pool)) +
  scale_y_continuous(breaks = c(-5:15,3)) +
  theme_minimal() +
  theme(legend.text = element_text(size=15), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
        axis.title.x = element_text(size = 15),axis.title.y = element_text(size = 15),
