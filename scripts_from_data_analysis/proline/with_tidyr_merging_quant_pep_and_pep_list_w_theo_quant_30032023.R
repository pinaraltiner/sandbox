...
final_proline_pep_quant_analysis <- function(file_path,
  ...
  quant_peptides <- read.xlsx(paste0(file_path_exp2,file_name_exp2), sheet = sheet_name)
  ...
  p3 <- gg_raincloud(data_set = figure_with_mean_abundance,
               x_df = figure_with_mean_abundance$Theo_Exp,
               y_df = figure_with_mean_abundance$Mean_abundance,
               fill_df = figure_with_mean_abundance$Pool,
               header = "Distribution of mean abundance of every sample with two different pools",
               x_lab = "Sample Names",
  ...
  p4 <- gg_boxplt_exp_ratio(data_set = df_for_figure, 
  ...
  p5 <- gg_half_boxplt_exp_ratio(data_set = df_for_figure, 
                                 x_df = df_for_figure$Theo_Exp,
                                 y_df = df_for_figure$Ratios,
                                 fill_df = df_for_figure$Pool,
                                 header="Experimental Quantity Ratio of Synthetic Peptides",
  ...
  p6 <- gg_violin_exp_ratio(data_set = df_for_figure, 
                            x_df = df_for_figure$Theo_Exp,
                            y_df = df_for_figure$Ratios,
                            fill_df = df_for_figure$Pool,
                            header="Experimental Quantity Ratio of Synthetic Peptides",
  ...
  sapply(1:6,function(x) ggsave(filename = paste0("p",x,".tiff"),
                                width = 50, height = 40, 
                                path = file_path,
                                units = "cm",
                                get(paste0("p",x)),
  ...
  df_for_ttest<- final_imputed_normalized_data %>%
    select(starts_with("mean_") | contains(c("Pool", "common_col_for_merging"))) %>%
    filter_at(vars(Pool), all_vars(!is.na(.))) #remove unexpectedly identified peptides
                                               # there is no meaning against theo pep list
  ...
  for(k in 2:sample_size){
    ...
    assign(paste0("p_values_for_ratio",k),
           as.data.frame(unlist(lapply(get(paste0("t_test_res_A1_to_A",k)),
                                       function (x) x[c('p.value')]))))
    
    assign(paste0("p_values_for_ratio",k),df_for_ttest %>%
             select(common_col_for_merging) %>%
             mutate(get(paste0("p_values_for_ratio",k))) %>%
             rename(p_values = 2) %>%
             arrange(desc(p_values)))
    
    assign(paste0("adj_t_test_res_A1_to_A",k),
           p.adjust(get(paste0("p_values_for_ratio",k))[,2],method = "BH",
                    n = dim(get(paste0("p_values_for_ratio",k))[2])))
    
    assign(paste0("adj_t_test_res_A1_to_A",k),df_for_ttest %>%
             select(common_col_for_merging) %>%
             mutate(get(paste0("adj_t_test_res_A1_to_A",k))))
  }
