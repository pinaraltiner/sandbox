...
a <- setwd("D:/dev/Desktop_copy/PHD/wet_lab_experiments/DDA_data_analysis/experiment_1/Proline/exp1_no_ecoli_no_FAIMS/")
list_files <- list.files(a)[-c(1:5,14:16)]
...
df_merge_localization <- df_rmv_redunc %>%
  ...
  ungroup() %>%
  full_join(pep_list_w_theo_quant_new,by=c("common_col_for_merging"))
...
#write.table(df_merge_localization,file=paste0(a,"/","PAL_Exp1_no_Ecoli_DDA_noFAIMS_localization_merge.tsv"),sep = "\t",row.names = F,col.names = T)
# 
#write.table(df_merge_sequence,file=paste0(a,"/","PAL_Exp1_no_Ecoli_DDA_noFAIMS_sequence_merge.tsv"),sep = "\t",row.names = F,col.names = T)
...
df_merge_sequence %>% select(pool_id_seq) %>% 
  ...
  # df_merge_sequence %>% select(pool_id_seq,ptm_score) %>% 
  # separate(pool_id_seq, into = c("sequence","pool_id"), sep = "_") %>%
  # #count(pool_id) %>%
  # ggplot(aes(y=ptm_score,x=sequence,color=pool_id)) +
  # geom_col() + 
  # theme_light() + #scale_y_continuous(limits = c(-60, 60),breaks = seq(-60, 60, by = 20)) +
  # theme(legend.text = element_text(size=20), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
  #       axis.title.x = element_text(size = 20),axis.title.y = element_text(size = 20),
