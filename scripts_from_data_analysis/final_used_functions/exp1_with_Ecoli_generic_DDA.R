...
pep_list_theo <- pep_list_w_theo_quant_new %>% left_join(map_file_raw_to_pool, by="pool_id") %>%
...
ecoli_df <-  as.data.frame(final_ecoli_count_df) %>% tibble() %>% mutate(comb_pool_inj = paste(Pool,inj_id, sep="_")) 
  
ecoli_df$num_id_Ecoli_peptides <- as.numeric(ecoli_df$num_id_Ecoli_peptides)
  
    
ggplot(ecoli_df,aes(y=num_id_Ecoli_peptides,
    ...
    geom_text(
      ...
      vjust = -0.5, size = 5) + 
    scale_y_continuous(limits=c(13000,14400)) +
...
df_merge_localization <- df_rmv_redunc %>%
  ...
  slice(which.max(ptm_score)) %>%
  ungroup() %>%
  full_join(pep_list_theo,by=c("common_col_for_merging"))
...
df_merge_sequence <- df_rmv_redunc %>%
  ...
  slice(which.max(ptm_score)) %>%
  ungroup() %>%
  full_join(pep_list_theo, by="rawfile_seq")
...
df_merge_sequence_type <- df_merge_sequence %>% 
 ...
 #write.table(df_merge_localization,file=paste0(a,"/","PAL_Exp1_with_coli_DDA_Exploris_npFAIMS_localization_merge.tsv"),sep = "\t",row.names = F,col.names = T)
# 
 #write.table(df_merge_sequence,file=paste0(a,"/","PAL_Exp1_with_coli_DDA_Exploris_npFAIMS_sequence_merge.tsv"),sep = "\t",row.names = F,col.names = T)


df_merge_localization_type %>%
  select(common_col_for_merging,
         pool_id.x,type) %>% 
  count(pool_id.x,type) %>% #drop_na() %>%
  ggplot(aes(x=pool_id.x,y=n,fill=type)) + 
  geom_col(position = "dodge",width = 0.98) + 
  geom_text(
    aes(x = pool_id.x, y = n, label = n, group = type),
    position = position_dodge(width = 1),
    vjust = -0.5, size = 5
  )+
...
df_merge_sequence_type %>%
  ...
  count(pool_id,type) %>% #drop_na() %>%
  ggplot(aes(x=pool_id,y=n,fill=type)) + 
  geom_col(position = "dodge",width = 0.98) + geom_text(
    aes(x = pool_id, y = n, label = n, group = type),
    position = position_dodge(width = 1),
    vjust = -0.5, size = 5
  )+
  theme_light() + #scale_y_continuous(limits = c(-60, 60),breaks = seq(-60, 60, by = 20)) +
...
df_merge_sequence_type %>% select(pool_id_seq,ptm_score,type,isomeric_count) %>% 
  separate(pool_id_seq, into = c("sequence","pool_id"), sep = "_") %>% 
  drop_na(ptm_score) %>%
  #count(pool_id) %>%
  ggplot(aes(y=pool_id,x= sequence)) +
  geom_point(aes(size=ptm_score,color=pool_id)) + 
  theme_light() + #scale_y_continuous(limits = c(-60, 60),breaks = seq(-60, 60, by = 20)) +
  theme(legend.text = element_text(size=20), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
        axis.title.x = element_text(size = 20),axis.title.y = element_text(size = 20),
        plot.title = element_text(size=20),
...
df_merge_localization_type %>% select(common_col_for_merging,pool_id,ptm_score,type) %>% 
  drop_na(ptm_score) %>%
  #count(pool_id) %>%
  ggplot(aes(y=common_col_for_merging,x=pool_id)) +
  geom_point(aes(size=ptm_score,color=pool_id)) + 
  theme_light() + #scale_y_continuous(limits = c(-60, 60),breaks = seq(-60, 60, by = 20)) +
  theme(legend.text = element_text(size=20), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
        axis.title.x = element_text(size = 20),axis.title.y = element_text(size = 20),
