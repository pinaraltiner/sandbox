...
map_file_name <- "Experiment1_no_Ecoli_noFAIMS_DDA_raw_files_pool_id.txt"
...
# 
#write.table(df_merge_sequence,file=paste0(a,"/","PAL_Exp1_no_Ecoli_DDA_noFAIMS_sequence_merge_with_raw_files.tsv"),sep = "\t",row.names = F,col.names = T)
...
df_merge_localization_type %>%
  ...
  theme(legend.text = element_text(size=20), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
        ...
        axis.text.x = element_text(size=20, angle = 90),
        axis.title=element_text(size=30)
  ) + ggtitle("Comparison of num. localized Syn. Phospho-peptides missing values") +
  labs(subtitle = "Experiment 1 no E.coli no FAIMS DDA proceesed by Proline")
...
df_merge_sequence_type %>% select(rawfile_seq,ptm_score,type) %>% 
  ...
  labs(subtitle = "Experiment 1 no E.coli no FAIMS DDA proceesed by Proline") +
  facet_wrap(~raw_files,ncol= 4)



### UNNECCESSARY TO CREATE IT, DENSITY PLOT VISUALIZES BETTER
# df_merge_localization_type %>% select(common_col_for_merging,rawfile_seq,ptm_score,type) %>% 
#   separate(rawfile_seq, into = c("sequence","raw_files"), sep = "\\.") %>%
#   #drop_na(ptm_score) %>%
#   #count(pool_id) %>%
#   ggplot(aes(y=common_col_for_merging,x=raw_files)) +
#   geom_point(aes(size=ptm_score,color=raw_files)) + 
#   theme_light() + #scale_y_continuous(limits = c(-60, 60),breaks = seq(-60, 60, by = 20)) +
#   theme(legend.text = element_text(size=20), #plot.margin=unit(c(-0.5,1,1,1), "cm"),
#         axis.title.x = element_text(size = 20),axis.title.y = element_text(size = 20),
...
df_merge_sequence %>% select(rawfile_seq) %>% 
  separate(rawfile_seq, into = c("sequence","raw_files"), sep = "\\.") %>%
  count(raw_files) %>%
  ggplot(aes(x=raw_files,y=n,fill=raw_files)) +
